...
mod tests {
    ...
    impl Fixture {
        fn new() -> Self {
            ...
            let hub = Hub::new(store, "test-host".into(), "test-token".into(), 19222);
            hub.start().unwrap();
    ...
    async fn delegation_roundtrip_keeps_queued_user_messages_separate() {
        ...
        let sent = crate::api::dispatch(&f.hub, "send", json!({"botId": "b", "text": "thanks"})).await.unwrap();
        let request_entry = f
            .hub
            .store
    ...
    async fn permission_cards_still_require_the_recipients_approval() {
        ...
        crate::api::dispatch(&f.hub, "respondPermission", json!({"entryId": card.id, "optionId": "allow"}))
            .await
            .unwrap();
    ...
    async fn stopping_requester_cancels_only_its_delegation() {
        ...
        f.until(|| f.prompts().len() == 1).await;
        crate::api::dispatch(&f.hub, "send", json!({"botId": "b", "text": "thanks"})).await.unwrap();
